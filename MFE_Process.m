...
param.MinPresence=20;       % Remove filaments that are present for less than x frames (default 30)
param.MaxGap=5;             % maximum number of frames an associated filament end is not present/detected
...
param.Square=10;            % size of square frame section around filament ends
param.BrightAdjust=.05;     %fraction adjustment for brightness cutoff (default 0.05)
...
param.CutOffVelocity=0.08;  % in mum/s   .03 for tonic, .15 testfiles
param.smooth=20;            % smoothing range, default 20 frames
param.MinSize=10;            % minimum size filament, in pixels. default 5
param.FrameAverage=4;       % number of frames to average. increase if noise dominates signal. default 2
param.DefaultThreshold=40;  % threshold value if no noise is present. default 40
param.MaxLength=1000;       % maximum duration for trace. low increases successful filament count
                            % but reduces ability to detect stopped vs. go,
                            % especially at low velocities.
%% end setting parameters

[frames, FilEnd, SizeFrame, num, Threshold,A]=VideoProcess_v3(dirout, file, param);
...
save([dirout '\results2\' file(1:end-4) '_ANL_Traced.mat'],'FilEnd','param','dirout','file','frames','VideoNow','num','Threshold','A')
%% end clean up trace
...
saveas(gca,[dirout '\results2\' file(1:end-4) '_result.png'])
saveas(gca,[dirout '\results2\' file(1:end-4) '_result.fig'])
figure(VideoNow+100)
...
hold on
plot(Stops,'r')
saveas(gca,[dirout '\results2\' file(1:end-4) '_result_filno.png'])
saveas(gca,[dirout '\results2\' file(1:end-4) '_result_filno.fig'])
close(VideoNow)
close(VideoNow+100)
save([dirout '\results2\' file(1:end-4) '_ANLpost.mat'],'TraceOut','FinalSpeeds','dirout','file','frames','FinalPosx','FinalPosy','Velocity','fmot','Lengths')
